...
自動更新每天都在增加集數，所以這裡的數字是**快照，不是即時值**。
最新數字看網站頁尾（「收錄至 ⋯ · N/M 集有分段時間戳」），
或執行 `python build.py` 看它印出的統計。
...
|---|---|
| 頻道影片總數 | 約 1,350 |
| 實際收錄 | **941 集**（2017–2026） |
| 有分段時間戳 | 686 集（73%） |
| 可搜尋的故事 | **9,804 則** |
| 索引體積 | 約 560KB，gzip 後約 265KB |

各年份的分段覆蓋率差很多，是撰稿習慣造成的（同為快照）：
...
> 影片（1,339 vs 1,271）。所以走 API 不只是為了避開封鎖，資料也更全。
